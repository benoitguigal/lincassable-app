CREATE OR REPLACE FUNCTION inventaire_trigger()
  RETURNS TRIGGER
  SECURITY DEFINER
  LANGUAGE 'plpgsql'
  AS $$
...
BEGIN
  ...
  FROM
    net.http_post(supabase_url || '/functions/v1/inventaire_trigger', jsonb_build_object('old_record', OLD, 'record', NEW, 'type', TG_OP, 'table', TG_TABLE_NAME, 'schema', TG_TABLE_SCHEMA), '{}', jsonb_build_object('Content-Type', 'application/json', 'Authorization', 'Bearer ' || supabase_api_key), '5000' -- timeout in ms
);
